...
export async function getMarketData(tokenConfig) {
    ...
    try {
        ...
        if (token) {
            ...
            for (let i = 0; i < pools.length; i++) {
                const pool = pools[i];
                const formattedPrice = Number(pool.token1Price);
                if (pool.token1.symbol === 'BUSD' && parseFloat(formattedPrice) < 10000) {
                    tokenPriceUsdFromBUSD = Number(pool.token1Price);
                    break;
                }
            }
            ...
            // Prefer token.derivedUSD if it's within a reasonable range, otherwise use BUSD pool price
            if (token.derivedUSD && Number(token.derivedUSD).toFixed(6) < 100000) {
                currentPriceFinal = safeParseFloat(Number(token.derivedUSD));
            } else if (tokenPriceUsdFromBUSD !== null) {
                currentPriceFinal = safeParseFloat(tokenPriceUsdFromBUSD);
            }
            ...
            if (currentPriceFinal === null || isNaN(currentPriceFinal)) {
                ...
                try {
                    ...
                    }, 3, 1000); // 3 retries, starting with 1 second delay (1s, 2s, 4s)
                ...
                } catch (dexscreenerRetryError) {
                    console.error(`Dexscreener initial price fetch failed after retries for ${targetTokenSymbol}:`, dexscreenerRetryError.message);
                    // If even retries fail, currentPriceFinal remains null/NaN, which is handled
                }
            }
